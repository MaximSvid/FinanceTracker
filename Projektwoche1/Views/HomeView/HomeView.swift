...
//        ]
    ...
    var body: some View {
        VStack {
            SheetDeleteCategory(isSheetDeleteCategoryPresented: $homeViewModel.isSheetDeleteCategoryPresented, categories: $homeViewModel.categories)
            AllMoney(wallet: wallets, context: modelContext)
            ListWalletHomeView(wallet: wallets)
            CategoryView(wallets: wallets, homeViewModel: homeViewModel)
            Spacer()
